...
describe('artifact-paths', () => {
  ...
  beforeEach(() => {
    originalHome = process.env.ORCHEX_HOME
  })
  ...
  afterEach(() => {
    if (originalHome === undefined) {
      ...
      process.env.ORCHEX_HOME = originalHome
    }
  })

  it('getTarballPath returns path under ORCHEX_HOME/tarballs', () => {
    const home = join(tmpdir(), randomUUID())
    process.env.ORCHEX_HOME = home
  ...
  it('getTarballPath uses .tar.gz extension', () => {
    process.env.ORCHEX_HOME = join(tmpdir(), randomUUID())
    expect(getTarballPath('my-pkg', '1.2.3')).toMatch(/\.tar\.gz$/)
  })

  it('getInstalledPackageDir returns path under ORCHEX_HOME/packages', () => {
    const home = join(tmpdir(), randomUUID())
    process.env.ORCHEX_HOME = home
