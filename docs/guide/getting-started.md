...
- `/using-spwnr` is the main router and should default to `/spwnr-plan`, then `/spwnr-task`, then implement.
- `/spwnr-do` handles a bounded small task directly and writes a lightweight note under `.claude/do/`.
...
- `/spwnr-worker-audit` checks whether the local agent setup is healthy and helps recover when worker selection fails.

Recommended usage inside Claude Code:

1. Open your project in Claude Code.
2. Start with `/using-spwnr` if you want the plugin to route you into the main plan-first flow automatically.
3. Use `/spwnr-do` when the request is a bounded follow-up fix, one-off operation, or single-goal edit that does not need full planning.
4. Run `/spwnr-plan` to clarify the goal and generate a plan when the work is broader or needs approval-gated execution.
5. If the plan looks good, continue with `/spwnr-task`.
6. Use Claude team features only when the approved plan selects `team`; `pipeline` can run without them.
7. If Claude cannot find suitable workers or your local setup looks incomplete, run `/spwnr-worker-audit`.
