...
主入口从执行型 workflow 切换为提示注入。`spec.instructions.system` 是主线必填字段，也是 host adapter 编译的核心输入。
...
- `spec.workflow` 允许存在，但仅作为 legacy metadata
- Spwnr 会校验 workflow 文件引用是否存在
- Spwnr 不执行 workflow，也不会把 workflow 编译成调度逻辑
...
- manifest 校验
- package 目录加载
- prompt/schema/skills/workflow 引用存在性检查
- 强校验 `spec.instructions.system`
