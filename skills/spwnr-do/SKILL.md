...
Use `spwnr-principle` only for shared normalization habits and sensible defaults. Do not inherit the full plan-first artifact workflow here.
Use `spwnr-worker-audit` only when worker coverage is weak or missing and `/spwnr-worker-audit` becomes the right recovery step.
...
- if coverage is weak or missing, stop and redirect to `/spwnr-worker-audit`
...
- If worker coverage is weak, missing, or unsupported for `claude_code`, stop and send the user to `/spwnr-worker-audit`.
