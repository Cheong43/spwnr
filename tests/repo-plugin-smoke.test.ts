...
describe('workflow docs', () => {
  ...
  it('encode plan-first execution guards, worker recovery, and helper-doc routing', () => {
    ...
    expect(doSkill).toContain('at most 3 workers');
    ...
    expect(workflowSkill).toContain('target command skill');
    expect(workflowSkill).toContain('targeted reads and edits');
    expect(workflowSkill).toContain('full thread history');
    ...
    expect(sessionStartHook).toContain('Load the relevant skill before acting');
    expect(sessionStartHook).toContain('read/edit targeted plan sections');
    expect(sessionStartHook).toContain('avoid passing full files or thread history');
    expect(sessionStartHook).not.toContain('TaskCreate');
    expect(sessionStartHook).not.toContain('Plan, Unit, Mode, Worktree, Blocked');
