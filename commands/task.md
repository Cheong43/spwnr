---
description: Validate the approved plan artifact, resolve the planned execution mode, then route execution through the workflow-task-orchestration skill.
---

# Spwnr Workflow Task Command

Use the `workflow-task-orchestration` skill as the `/spwnr:task` router.
...
- validate that `Execution Strategy Recommendation` explicitly chooses `pipeline` or `team`, records the rationale, and persists the execution pattern before any routing
