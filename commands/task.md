...
- if the current run has not yet received `执行当前计划`, use the same execution review loop with `AskUserQuestion` before orchestration
- if the user chooses `继续改进计划`, revise the same active revision when the execution shape is unchanged, or create the next revision file when the request becomes a material re-plan
...
- use `single-lane`, `team`, or `swarm` execution only after the current run has received `执行当前计划`
